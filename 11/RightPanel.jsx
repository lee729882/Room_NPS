      ...
      <div className="p-5 bg-white border-b border-slate-100 shrink-0">
        ...
        <div className="flex items-start gap-1 mb-3">
          ...
          <h2 className="text-[13px] font-black text-[#0f172a] tracking-tighter leading-tight">
            [{details.bldNm || selectedBuilding.title || '정보 없음'}] 종합 진단
          </h2>
        </div>
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm space-y-3">
            ...
            <div className={`rounded-xl px-4 py-2.5 border text-[10px] font-bold ${ratioCls}`}>
              <div className="flex justify-between items-center mb-1.5">
                <span className="font-black">보증금 보호 점수 :</span>
                {officialTotal > 0 && (
                  <span className="bg-white/50 px-2 py-0.5 rounded text-[9px] font-black tracking-tight">
                    공시지가: {new Intl.NumberFormat('ko-KR').format(officialTotal)}만원
                  </span>
                )}
              </div>
              <p className="leading-relaxed opacity-90">
                {details.ratioDiagnosis || (officialTotal > 0 ? '전세가율 분석 완료' : '공시지가 데이터 확인 필요')}
              </p>
            </div>
