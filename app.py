...
def generate_ai_report():
    ...
    price_anomaly = data.get('priceAnomaly', {})
    risk_cluster = data.get('riskCluster', {})
    anomaly_msg = price_anomaly.get('message', '') if price_anomaly.get('available') else ''
    cluster_label = risk_cluster.get('clusterLabel', '') if risk_cluster.get('available') else ''


    # 프롬프트 구성 (구조 고정)
    prompt = f"""
대한민국 주택임대차 전문 변호사이자 원주시 청년 정주지원 자문가로서, 다음 구조에 맞춰 매물 진단 리포트를 작성하세요. 반드시 아래 형식을 엄격히 준수해야 합니다.

1. **[{building_name}] AI 안심 진단 요약**:
- 전세가율이 0이거나 정보가 없으면 "전세 실거래 데이터가 부족해 전세가율 판정을 보류합니다"라고 명시하고, 절대 "0%라 안정적"이라고 표현하지 마세요.
...
3. **⚠️ NPS 데이터 기반 리스크 진단**:
   - 위반건축물 여부({'있음' if is_violation else '없음'})를 바탕으로 법적 리스크를 확정적으로 언급하세요. 전세자금대출이나 보증보험 가입 가능 여부를 명시하세요.
   - AI 이상치 분석 결과("{anomaly_msg}")와 위험유형 분류("{cluster_label}")를 근거로, 이 매물이 주변 매물 대비 어떤 위치인지 함께 설명하세요. (해당 데이터가 비어있으면 이 부분은 생략)

3-1. **🏥 보건의료 접근성 안내**:  # [FIX-5]
   - 인근 응급실·달빛어린이병원·야간약국 접근성은 계약 전 직접 확인을 권장합니다.
   - 국립중앙의료원 응급의료정보 제공 서비스(www.e-gen.or.kr)에서 인근 기관을 조회할 수 있습니다.

4. **🏠 원주시 청년 정주지원 행동 조언**:
   - 위 [원주시 청년 주거지원 정책 컨텍스트]를 참고하여, 만약 사용자가 만 18~39세 무주택 청년에 해당할 가능성이 있다면 '청년 전세보증금반환보증 보증료 지원사업' 신청을 검토해보라고 안내하세요.
   - 전세가율이 높거나(caution/danger 등급) 위반건축물이 있는 경우에는 특히 보증보험 가입 및 해당 지원사업 활용을 강조하세요.
- 신청 문의처(원주시청 주택과 주택정책팀 033-737-3474 또는 국토교통부 전세보증금반환보증 보증료 지원 콜센터 1599-0001)를 안내하고, 정확한 지원 한도와 신청기간은 최신 공고를 확인하라는 안내를 반드시 포함하세요.
[매물 데이터]
- NPS 점수: {nps_score}점
- 전세가율: {jeonse_ratio}%
- 위반건축물: {'있음' if is_violation else '없음'}
- AI 이상치 분석: {anomaly_msg if anomaly_msg else '데이터 부족'}
- AI 위험유형 분류: {cluster_label if cluster_label else '데이터 부족'}

[법률 컨텍스트]
{LEGAL_CONTEXT[:1500]}

[원주시 청년 주거지원 정책 컨텍스트]
{WONJU_POLICY_CONTEXT[:1000]}

결과는 반드시 Markdown 형식으로 출력하되, 아래 규칙을 지키세요:
- 각 섹션 제목은 ### (h3)로 시작하세요.
- 각 섹션 사이에는 반드시 빈 줄을 두 개 넣으세요.
- 각 섹션 본문은 2~3문장으로 짧게 쓰고, 핵심 수치는 **굵게** 표시하세요.
- 한 문단이 3문장을 넘지 않게 하세요.
- 절대로 "~을 설명하세요", "~을 명시합니다", "~을 언급합니다", "~을 안내합니다" 같은 지시문을 그대로 답변에 쓰지 마세요. 반드시 실제 분석 결과와 결론을 완성된 문장으로 작성하세요.
- 데이터가 부족한 항목은 "관련 데이터가 부족하여 판단을 보류합니다"라고 명확히 쓰고, 억지로 지어내지 마세요.
불필요한 서론이나 결론은 생략하세요.
"""
    ...
    try:
        completion = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "당신은 어려운 법률 용어를 쉽게 풀어 설명해주는 친절하고 전문적인 부동산 전문 변호사입니다. 프롬프트의 지시문을 그대로 반복하지 말고, 반드시 실제 분석 내용과 결론을 완성된 문장으로 작성하세요. 결과는 마크다운 형식으로 군더더기 없이 제공하세요."},               
...
TAAS_KEY = os.getenv("TAAS_API_KEY")
TAAS_LG_ENDPOINT = "https://opendata.koroad.or.kr/data/rest/frequentzone/lg"  # [FIX-2] TAAS 교통사고 다발지역 엔드포인트 상수화
BACKEND_URL = os.getenv("BACKEND_URL", "http://localhost:5000")  # [FIX-1] 환경변수화
...
# ─────────────────────────────────────────────────────────────
# [FIRE-1] 소방청 화재통계 CSV 로드 (서버 시작 시 1회)
# ─────────────────────────────────────────────────────────────
try:
    FIRE_CSV_PATH = os.path.join(os.path.dirname(__file__), '소방청_연간화재통계_20241231.csv')
    FIRE_DF = pd.read_csv(FIRE_CSV_PATH, encoding='cp949', low_memory=False)
    log.info(f"[FIRE-CSV] 소방청 화재통계 데이터 로드 완료: {len(FIRE_DF)}건")
except Exception as e:
    FIRE_DF = None
    log.warning(f"[FIRE-CSV] 로드 실패: {e}")

# [FIRE-2] 원주시 연간 화재 건수 집계 (시작 시 1회 계산)
# 컬럼명: '시도', '시_군_구' (소방청 CSV 실제 헤더 기준)
try:
    if FIRE_DF is not None:
        _wonju_mask = FIRE_DF['시도'].str.contains('강원', na=False) & (FIRE_DF['시_군_구'] == '원주시')
        WONJU_FIRE_COUNT = int(_wonju_mask.sum())
        log.info(f"[FIRE-CSV] 원주시 연간 화재 {WONJU_FIRE_COUNT}건")
    else:
        WONJU_FIRE_COUNT = None
except Exception as e:
    WONJU_FIRE_COUNT = None
    log.warning(f"[FIRE-CSV] 원주 건수 집계 실패: {e}")

# [FIRE-3] 지역 화재 건수 → 0~100 점수 변환
# 구간: 150건 미만=85, 150~300=70, 300~450=55(원주 308건 해당), 450 이상=45
if WONJU_FIRE_COUNT is not None:
    if WONJU_FIRE_COUNT < 150:
        region_fire_score = 85
    elif WONJU_FIRE_COUNT < 300:
        region_fire_score = 70
    elif WONJU_FIRE_COUNT < 450:
        region_fire_score = 55
    else:
        region_fire_score = 45
else:
    region_fire_score = None
...
# ── 원주시 청년 주거지원 정책 컨텍스트 로드 ──
WONJU_POLICY_CONTEXT = ""
try:
    WONJU_POLICY_PATH = os.path.join(os.path.dirname(__file__), '원주시_청년주거지원정책.txt')
    if os.path.exists(WONJU_POLICY_PATH):
        with open(WONJU_POLICY_PATH, 'r', encoding='utf-8') as f:
            WONJU_POLICY_CONTEXT = f.read()
        log.info(f"[POLICY] 원주시 청년주거지원정책 컨텍스트 로드 완료 ({len(WONJU_POLICY_CONTEXT)} bytes)")
    else:
        log.warning("[POLICY] 원주시_청년주거지원정책.txt 파일을 찾을 수 없습니다.")
except Exception as e:
    log.error(f"[POLICY] 로드 실패: {e}")
...
# ── 교통사고 다발지역 전역 캐시 (서버 시작 시 1회 로드) ──
TAAS_ACCIDENTS = []   # fetch_wonju_traffic_accidents() 결과 저장
TAAS_LOADED    = False

def ensure_taas_loaded():
    """교통사고 데이터가 로드되지 않았으면 지금 로드한다."""
    global TAAS_ACCIDENTS, TAAS_LOADED
    if not TAAS_LOADED:
        log.info("[TAAS] 교통사고 다발지역 데이터 로딩 시작...")
        TAAS_ACCIDENTS = fetch_wonju_traffic_accidents()
        TAAS_LOADED = True
        log.info(f"[TAAS] 캐시 완료: {len(TAAS_ACCIDENTS)}건")
    return TAAS_ACCIDENTS
...
def fetch_safety_data(lat, lng, sigungu_cd=""):
    ...
    try:
        ...
        # [FIRE-4] 소방서 거리 기반 점수 (기존 로직 유지, dist_fire_score로 명명)
        dist_fire_score = int(max(0, 100 - 60 * ((f_dist - 300) / 1200) ** 0.5)) if f_dist is not None else 65
        # [FIRE-4] 최종 fire_score: 소방서 거리 50% + 원주 지역 화재량 50% 혼합
        # region_fire_score가 없으면(CSV 미로드) 기존 방식 폴백
        if region_fire_score is not None:
            fire_score = int(dist_fire_score * 0.5 + region_fire_score * 0.5)
        else:
            fire_score = dist_fire_score
        ...
        return {
            ...
            "radar": [  # [FIX-3] 레이더 차트 항목명 제안서 기준으로 변경
                {"subject": "범죄안전", "score": security_score},
                {"subject": "화재안전", "score": fire_score},
                {"subject": "보건의료안심", "score": risk_score},
                {"subject": "야간교통안전", "score": min(100, max(30, 75 - (seed % 20)))},
...
def fetch_wonju_traffic_accidents():
    """
    도로교통공단 지자체별 교통사고 다발지역 API(/frequentzone/lg)를 통해
    원주시(강원도 시도코드=42, 시군구코드=130) 데이터를 조회합니다.
    - 연도: 2017~2024 전체 수집 (데이터 있는 연도만 포함)
    - 파라미터: siDo/guGun 카멜케이스, 실테스트로 확인된 방식
    - 전 연도 데이터를 합산하여 반환 (spot_cd 기준 중복 제거)
    """
    import urllib3
    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

    if not TAAS_KEY:
        log.warning("[TAAS-LG] TAAS_API_KEY가 설정되지 않았습니다.")
        return []

    endpoint   = TAAS_LG_ENDPOINT  # [FIX-2] 하드코딩 제거 → 상수 참조
    SIDO_CODES = ["42", "51"]   # 강원도(우선) / 강원특별자치도
    GUGUN_CODE = "130"          # 원주시
    ALL_YEARS  = [str(y) for y in range(2024, 2016, -1)]  # 2024~2017
    ...
    all_accidents = {}  # spot_cd → item (중복 제거용)
    ...
    for sido in SIDO_CODES:
        for year in ALL_YEARS:
            try:
                ...
                log.debug(f"[TAAS-LG] 요청: {year}년 siDo={sido}")

                r = req_lib.get(full_url, headers=headers, verify=False, timeout=10)
                if r.status_code != 200:
                    continue
                ...
                try:
                    ...
                    log.warning(f"[TAAS-LG] {year}/sido={sido} JSON 파싱 실패")
                    continue

                result_code = str(res_json.get("resultCode", "")).strip()
                total_count = safe_int(res_json.get("totalCount", 0))

                if result_code == "03" or total_count == 0:
                    continue  # 해당 연도/시도 데이터 없음

                if result_code not in ("00", ""):
                    log.warning(f"[TAAS-LG] {year}/sido={sido} 오류: code={result_code}")
                    continue

                items_raw = res_json.get("items") or {}
                ...
                for it in item_list:
                    try:
                        lat_val = it.get("la_crd") or ""
                        lng_val = it.get("lo_crd") or ""
                        if not lat_val or not lng_val:
                            continue
                        spot_key = it.get("spot_cd") or f"{lat_val}_{lng_val}"
                        # 이미 있으면 더 최신 연도(occnt 높은 것)로 갱신
                        existing = all_accidents.get(spot_key)
                        new_occnt = safe_int(it.get("occrrnc_cnt", 0))
                        if existing and safe_int(existing.get("occnt", 0)) >= new_occnt:
                            continue
                        all_accidents[spot_key] = {
                            "name":   it.get("spot_nm", "원주시 교통사고 다발지역"),
                            "lat":    float(lat_val),
                            "lng":    float(lng_val),
                            "occnt":  new_occnt,
                            "cascnt": safe_int(it.get("caslt_cnt", 0)),
                            "dthcnt": safe_int(it.get("dth_dnv_cnt", 0)),
                            "seCnt":  safe_int(it.get("se_dnv_cnt", 0)),
                            "slCnt":  safe_int(it.get("sl_dnv_cnt", 0)),
                            "year":   year,
                            "sido":   sido,
                            "spotCd": spot_key,
                        }
                        ...
                        log.warning(f"[TAAS-LG] 아이템 파싱 오류: {e}")
                        continue

                if count_added > 0:
                    log.info(f"[TAAS-LG] {year}년/sido={sido} → {count_added}건 수집")

            except Exception as e:
                log.error(f"[TAAS-LG] {year}/sido={sido} 예외: {e}")
                continue

    result = list(all_accidents.values())
    log.info(f"[TAAS-LG ✓] 전년도 합산 완료: 총 {len(result)}개 다발지역")
    return result


def get_traffic_accident_score(accidents, lat, lng, radius_km=1.5):
    """
    매물 위치(lat, lng) 기준 반경 radius_km 이내 교통사고 다발지역을 필터링하고
    NPS 교통안전 점수(0~100)와 근접 사고지역 목록을 반환합니다.

    점수 산정:
    - 반경 내 사고지역이 없으면 100점
    - 사고건수 가중치: 각 지역의 occnt * 거리 역수 가중치
    - 사망자 포함 시 추가 패널티
    """
    if not accidents:
        return 100, []
    ...
    for acc in accidents:
        dist = get_distance(lat, lng, acc["lat"], acc["lng"])
        if dist <= radius_km:
            nearby.append({**acc, "distKm": round(dist, 3)})
    ...
    risk_index = 0.0
    for acc in nearby:
        dist_m = acc["distKm"] * 1000
        weight = 1.0 / max(dist_m, 50)       # 최소 거리 50m 보정
        risk_index += acc["occnt"] * weight
        risk_index += acc["dthcnt"] * weight * 5  # 사망자 5배 패널티

    # 점수 변환 (risk_index 0~0.5 → 점수 100~30)
    score = max(30, min(100, int(100 - risk_index * 140)))
    nearby.sort(key=lambda x: (x["distKm"], -x["occnt"]))
    return score, nearby
...
def analyze():
    try:
        ...
        if bjd_code:
            if bjd_code.startswith('42'):
                bjd_code = '51' + bjd_code[2:]
            elif bjd_code.startswith('45'):
                bjd_code = '52' + bjd_code[2:]
        ...
        # ── 교통사고 다발지역 데이터 로드 및 교통안전 점수 산출 ──
        accidents_all = ensure_taas_loaded()
        traffic_score, nearby_accidents = get_traffic_accident_score(accidents_all, lat, lng, radius_km=1.5)
        log.info(f"[analyze] 교통안전 점수={traffic_score} | 반경 내 다발지역={len(nearby_accidents)}건")

        # 범죄/치안 점수(60%) + 교통안전 점수(40%) → 통합 안전 점수  # [FIX-4] 주석 실제 비율로 수정
        combined_safety_score = int(safety['securityScore'] * 0.6 + traffic_score * 0.4)

        # 레이더 차트 야간교통안전 항목 실제 점수로 업데이트
        radar = safety.get('radar', [])
        for rd in radar:
            if rd.get('subject') == '야간교통안전':  # [FIX-3] subject명 일치
                rd['score'] = traffic_score
        ...
        nps = compute_nps(
            ...
            security_score=combined_safety_score,
        ...
        return jsonify({
            ...
            "trafficAccidents": nearby_accidents[:10],  # 최대 10개
            "trend": trend_data,
            "safemap": safety,
            "building": {
            ...
            "diagnosis": {
                ...
                "traffic":   (
                    f"반경 1.5km 내 교통사고 다발지역 {len(nearby_accidents)}곳 확인됨 — 교통안전 점수 {traffic_score}점"
                    if nearby_accidents else
                    "반경 1.5km 내 교통사고 다발지역 없음 — 교통 안전 구간"
                ),
...
if __name__ == '__main__':
    load_geo_cache()
    # print("TAAS 테스트 데이터:", fetch_wonju_traffic_accidents())
